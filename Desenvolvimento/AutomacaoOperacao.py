...
# Função para filtrar registros de 4 dias atrás em diante
def filter_data_from_date(df, date_column):
    four_days_ago = datetime.now() - timedelta(days=4)
    # Garantir que o tipo da coluna de data seja datetime
    if date_column in df.columns:
        df[date_column] = pd.to_datetime(df[date_column], errors='coerce')
    
        # Filtrar os dados onde a coluna de data é maior ou igual a 4 dias atrás
        return df[df[date_column] >= four_days_ago.date()]
    else:
        print(f"Coluna '{date_column}' não encontrada no DataFrame.")
        return df
...
# Executar a deleção dos registros antigos
delete_sql = """
DELETE FROM stages_rc_card.compras_clientes
WHERE "Data" = CURRENT_DATE - INTERVAL '4 days';
"""
try:
    with engine.connect() as connection:
    ...
    print(f"Erro ao deletar registros antigos: {e}")

# Caminho para a pasta com arquivos Excel
folder_path = '//srv/ie/Planilhas/ComprasClientes/Automacao'
...
for file in files:
    ...
    try:
        ...
        # Filtrar registros com a data de 4 dias atrás em diante
        df = filter_data_from_date(df, 'Data')
...
if planilhas:
    ...
    print("Nenhum dado correspondente à data de 4 dias atrás ou posterior foi encontrado.")